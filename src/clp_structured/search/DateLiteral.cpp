    ...
    std::shared_ptr<Literal> DateLiteral::create_from_string(std::string const& v) {
        ...
        if (false == ss.fail() && ss.eof()) {
            return std::shared_ptr<Literal>(
                    static_cast<Literal*>(new DateLiteral(tmp_double_epoch, v))
            );
        }
