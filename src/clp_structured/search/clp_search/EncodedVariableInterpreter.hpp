#ifndef CLP_STRUCTURED_SEARCH_CLP_SEARCH_ENCODEDVARIABLEINTERPRETER_HPP
#define CLP_STRUCTURED_SEARCH_CLP_SEARCH_ENCODEDVARIABLEINTERPRETER_HPP
...
#include "src/clp_structured/DictionaryWriter.hpp"
#include "src/clp_structured/TraceableException.hpp"

namespace clp_structured {
/**
 * Class to parse and encode strings into encoded variables and to interpret encoded variables back
 * into strings. An encoded variable is one of: i)   a variable dictionary ID, referring to an entry
 * in the variable dictionary, or ii)  a value, representing an integer variable exactly as it
 * appears in the original log message, or iii) a value, representing a base-10, 16-digit number
 * with a decimal point, where at least one digit is after the decimal point, encoded with a custom
 *      format.
 *
...
#endif  // CLP_STRUCTURED_SEARCH_CLP_SEARCH_ENCODEDVARIABLEINTERPRETER_HPP
