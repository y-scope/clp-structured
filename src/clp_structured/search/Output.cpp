...
#include "submodules/nlohmann_json/include/nlohmann/json.hpp"
...
void Output::init(
        SchemaReader* reader,
        int32_t schema_id,
        std::vector<BaseColumnReader*>& columns
) {
    m_reader = reader;
    m_schema = schema_id;
    ...
    for (auto* column : columns) {
        ...
        VariableStringColumnReader* var_reader = dynamic_cast<VariableStringColumnReader*>(column);
