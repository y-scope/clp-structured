...
EvaluatedValue EvaluateTimestampIndex::run(std::shared_ptr<Expression> const& expr) {
    if (std::dynamic_pointer_cast<OrExpr>(expr)) {
             ...
             range_it++)
        {
            ...
            // TODO: handle wildcard matching; the initial check on timestamp index happens before
            // schema matching, so
            ...
            EvaluatedValue ret;
            // this is safe after type narrowing because all DateType literals are either Integral
            // or a derived class of Integral
